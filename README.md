...
- **amoCRM** — заказы автоматически создают сделки во внешней CRM
...
1. Создайте аккаунт на [amocrm.ru](https://www.amocrm.ru)
2. **amoМаркет → Создать интеграцию → Внешняя интеграция** → скопируйте долгосрочный токен
3. Добавьте в `.env` или Render:
...
`AMOCRM_PIPELINE_ID` — ID воронки (опционально). В URL воронки: `/pipeline/123456`.

При заказе создаётся сделка «Заказ #N — ScentForge», контакт клиента и примечание с товарами.

Админ: меню **«CRM»** на сайте → открывается amoCRM.
